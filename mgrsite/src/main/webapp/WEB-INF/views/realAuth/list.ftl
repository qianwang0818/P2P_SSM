...
<script type="text/javascript">
	$(function() {
		...
		//审核(包括审核通过和审核拒绝)按钮的点击事件
		$(".btn_audit").click(function(){
			var form=$("#editform");
			form.find("[name=state]").val($(this).val());		//把按下的审核按钮结果是通过or拒绝,传递到隐藏input中
			$("#myModal").modal("hide");						//把模态框关了,接着马上就提交
			...
			<div class="col-sm-9">
				...
				<div class="panel panel-default">
					<table class="table">
						...
						<tbody>
						<#list pageResult.listData as realAuth>
							<tr>
								<td>${realAuth.applier.username}</td>
								<td>${realAuth.realName}</td>
								<td>${realAuth.sexDisplay}</td>
