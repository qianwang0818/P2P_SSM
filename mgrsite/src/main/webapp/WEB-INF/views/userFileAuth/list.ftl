...
<script type="text/javascript">
	$(function() {
        ...
        $(".auditClass").click(function(){
            ...
            $("#file").attr("src",json.file);
            $("#myModal").modal("show");	//填充完数据,打开模态框
        });

        //查看按钮的点击事件
        $(".viewClass").click(function(){
            var json=$(this).data("json");
            $("#viewapplier").text(json.applier);
            $("#viewfileType").text(json.fileType);
            $("#viewscore").text(json.score);
            $("#viewremark").text(json.remark);
            $("#viewfile").attr("src",json.file);
            $("#viewModal").modal("show");	//填充完数据,打开模态框
        });
						...
						<#list pageResult.listData as info>
							<tr>
								...
								<td>
									...
									<#else>
										<a href="javascript:void(-1);" class="viewClass" data-json='${info.jsonString}'>查看</a>
									</#if>
								</td>
							</tr>
						</#list>
		      ...
		      <div class="modal-body">
		      	<form class="form-horizontal" id="editform" method="post" action="/userFile_audit.do">
					<fieldset>
			        	...
			        	<div class="col-sm-6">
			        		<img src="" id="file" style="height: 200px;"/>
			        	</div>
			        	</div>
			        	<div class="form-group">
		    ...
		    <div class="modal-content">
		      <div class="modal-body">
				  <form class="form-horizontal">
					<fieldset>
						<div id="legend" class="">
							<legend>风控材料查看</legend>
						</div>
						<div class="form-group">
							<label class="col-sm-2 control-label" for="name">用户名</label>
							<div class="col-sm-6">
								<label class="form-control" id="viewapplier"></label>
							</div>
						</div>
						<div class="form-group">
							<label class="col-sm-2 control-label" for="name">资料类型</label>
						<div class="col-sm-6">
							<label class="form-control" id="viewfileType"></label>
						</div>
						</div>
						<div class="form-group">
							<label class="col-sm-2 control-label" for="name">资料图片</label>
							<div class="col-sm-6">
								<img src="" id="viewfile" style="height: 200px;"/>
							</div>
						</div>
						<div class="form-group">
							<label class="col-sm-2 control-label" for="name">征信分数</label>
							<div class="col-sm-6">
								<label class="form-control" id="viewscore"></label>
							</div>
						</div>
						<div class="form-group">
							<label class="col-sm-2 control-label" for="name">审核备注</label>
							<div class="col-sm-6">
								<textarea id="viewremark" rows="4" cols="60" disabled></textarea>
							</div>
						</div>
					</fieldset>
				  </form>
